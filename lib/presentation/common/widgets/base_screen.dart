...
import 'package:soccer/presentation/common/widgets/app_state_views.dart';
...
class BaseAsyncScreen<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BaseScreen(
      ...
      background: background,
      body: switch (value) {
        AppSuccess<T>(data: final result) => () {
          if (result == null) {
            return AppErrorView(message: 'Không có dữ liệu', onRetry: onRetry);
          }
        ...
        AppLoading<T>() => loading ?? const AppLoadingView(),
        AppInitial<T>() => loading ?? const AppLoadingView(),
        _ => loading ?? const AppLoadingView(),
      },
    );
  }
}
